...
async function main() {
  ...
  const crossListLookup = sectionsFromDb.reduce(
    (crossListLookup, section) => {
      const course = crossListLookup[Model.courseKey(section)] || {
      ...
      crossListLookup[Model.courseKey(course)] = course;
